...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if is_pruned:
            ...
            tmp_tensor = identity[:,0:1,:,:].expand(x.shape[0], self.out_channels, identity.shape[2], identity.shape[3]).clone().fill_(0)

            tmp_tensor[:,indices[0],:,:] += out
            tmp_tensor[:,indices[1],:,:] += identity
            out = tmp_tensor[:, all_indices, :, :]
...
class BottleneckResNetImagenet(nn.Module):
    def __init__(self, net_prev, fix_rotation=True):
        super(BottleneckResNetImagenet, self).__init__()
        self.conv1 = net_prev.conv1
        self.bn = net_prev.bn1
        self.layer1 = net_prev.layer1
        self.layer2 = net_prev.layer2
        self.layer3 = net_prev.layer3
        self.layer4 = net_prev.layer4
        self.linear = net_prev.fc
